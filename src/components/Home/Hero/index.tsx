...
import { Dialog, NavButton, NavigationList } from '@/components/Header/navbar'
  ...
  return (
    <div className='bg-white'>
      <div className='relative isolate overflow-hidden bg-gradient-to-b from-[#1eb2a6]/10 '>
        <div
          className={`mx-auto max-w-[1500px] flex h-full w-full justify-between  px-3 py-5 lg:px-10 lg:py-6 max-md:max-w-full max-md:flex-wrap md:justify-center xl:justify-between`}
        >
          ...
          <div className='my-auto hidden  grow items-center justify-center md:gap-x-5 lg:gap-x-7 xl:gap-x-12 self-start text-sm lg:text-base font-medium leading-5 text-zinc-800 max-md:max-w-full md:flex md:text-[13px]'>
            {NavigationList.map((item) => (
              <NavButton
                className='after:hover:bg-custom-color leading-6 font-medium'
                key={item.name}
                name={item.name}
                href={item.href}
              />
          ...
          <div className='hidden items-center max-md:max-w-full max-md:flex-wrap md:flex'>
            <NavButton
              name='APPLY NOW'
              className='after:hover:w-0 bg-custom-color  py-3 px-4 rounded-md text-white hover:text-custom-color hover:bg-transparent border border-custom-color duration-300 md:text-xs lg:text-base'
              href='/registration'
            />
          </div>
        </div>
        ...
        <div className='mx-auto max-w-7xl  px-6 py-10 lg:py-28 lg:px-8'>
          <div className='mx-auto max-w-2xl lg:mx-0 lg:grid lg:max-w-none lg:grid-cols-2 lg:gap-x-16 lg:gap-y-6 xl:grid-cols-1 xl:grid-rows-1 xl:gap-x-8'>
            <h1 className='max-w-2xl text-4xl font-bold tracking-tight text-custom-color sm:text-6xl lg:col-span-2 xl:col-auto'>
